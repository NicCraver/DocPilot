...
已实现 `Tool` trait、`ToolRegistry`、统一类型（`ToolSchema` / `ToolInput` / `ToolOutput` / `ToolError`），共 **23** 个注册工具（PDF 10 + 图片 8 + 文件 4 + 文本 1），含共享 util（`pdf_lopdf_util` / `image_util` / `file_util`）与 `extra/` 批量工具模块。

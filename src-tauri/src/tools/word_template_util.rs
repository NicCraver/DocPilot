...
use crate::tools::word_typeset_util::TypesetBatchResult;
...
fn script_path() -> Result<PathBuf, String> {
    for root in project_root_candidates() {
        let root = root.canonicalize().unwrap_or(root);
        let script = root.join("scripts").join("word-template-fill.py");
        if script.is_file() {
            return Ok(script);
        }
    }
    Err("未找到 scripts/word-template-fill.py".into())
}

pub fn run_word_template_fill(payload: serde_json::Value) -> Result<TypesetBatchResult, String> {
    ...
    let output = Command::new(&python)
        ...
        .arg(&json)
        .output()
        .map_err(|e| format!("无法启动 Word 模板生成脚本 ({e})。{INSTALL_HINT}"))?;
    ...
    Err(if detail.is_empty() {
        format!("Word 模板生成失败（退出码 {:?}）", output.status.code())
    } else {
        format!("Word 模板生成失败: {detail}")
    })
}
